...
def test_ttw_energy_against_VECTO():
    ...
    assert vecto_empty <= tm.array.sel(powertrain="ICEV-d",
                        year=2020,
                        size="40t",
                        parameter="TtW energy", value=0) <= vecto_full
...
def test_fuel_blends():
    ...
    for fuel in tm.fuel_blend:
        np.testing.assert_array_equal(
            np.array(tm.fuel_blend[fuel]["primary"]["share"]) + np.array(tm.fuel_blend[fuel]["secondary"]["share"]),
            [1, 1, 1, 1, 1, 1]
        )
...
def test_noise_emissions():
    ...
    params = [p for p in tm.array.parameter.values
